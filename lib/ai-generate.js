...
function isOriginAllowed(origin, allowedList) {
  ...
  if (allowedList.indexOf(origin) !== -1) return true;

  // ワイルドカード対応: https://*.vercel.app のような形式を許可
  for (var i = 0; i < allowedList.length; i++) {
    var rule = String(allowedList[i] || "").trim();
    if (!rule) continue;
    if (rule.indexOf("*") === -1) continue;
    if (!rule.startsWith("http://") && !rule.startsWith("https://")) continue;
    try {
      var originUrl = new URL(origin);
      var ruleUrl = new URL(rule.replace("*.", "placeholder."));
      var expectedProtocol = ruleUrl.protocol;
      var expectedHostPattern = ruleUrl.hostname.replace("placeholder.", "*.");
      if (originUrl.protocol !== expectedProtocol) continue;
      if (!expectedHostPattern.startsWith("*.")) continue;
      var suffix = expectedHostPattern.slice(1); // ".vercel.app"
